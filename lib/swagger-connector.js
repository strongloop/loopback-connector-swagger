...
SwaggerConnector.prototype.setupDataAccessObject = function() {
  ...
  for (var a in this.client.apis) {
    ...
    if (api.operations) {
      ...
      /* var definition = this.client.definitions[a];
      var model = this.dataSource.app.models[a];
      var Model = this.dataSource.createModel(a, schema.properties, schema.options);
      model = model.extend(model.modelName, {someProperty: {type: 'number'}});*/
      var definition = this.client.definitions[a];

      function swaggerTypeToLoopbackType(type, format, ref) {
        switch (type) {
          case undefined:
            switch (ref) {
              case '#/definitions/GeoPoint':
                return 'geopoint';
              default:
                return 'object';
            }
      ...
      var props = Object.keys(definition.properties).reduce((result, key)=>{
        var prop = definition.properties[key];
        var mappedProp = {
          type: swaggerTypeToLoopbackType(prop.type, prop.format, prop.$ref),
          required: (definition.required || []).some(s=> s === key),
        };
        result[key] = mappedProp;
        return result;
      }, {});

      var _model = loopback.createModel(this.settings.name + '_' + a, props, {base: 'Model'});
      this.dataSource.app.model(_model, {
        dataSource: null,
      });
    }
  }
