...
SwaggerConnector.prototype.setupDataAccessObject = function() {
  ...
  for (const a in this.client.apis) {
    const api = this.client[a];

    for (const o in api.operations) {
      const method = api[o];
      const methodName = this._methodName(a, o, this.DataAccessObject);

      if (debug.enabled) {
        debug('Adding method api=%s operation=%s as %s', a, o, methodName);
      }

      const wrapper = createCallbackWrapper(method);
      const swaggerMethod = wrapper.bind(this.client);
...
SwaggerConnector.prototype.setupConnectorHooks = function() {
  ...
  self.connectorHooks.beforeExecute.apply = function(obj) {
    ...
    const cbSuccess = obj.on.response;
    const cbError = obj.on.error;
